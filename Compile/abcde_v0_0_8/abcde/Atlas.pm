...
my $varType = qr/COUNTER|CUSTOMPOINTER|POINTERTABLE|POINTERLIST|EMBPOINTERTABLE|TABLE|EXTENSION/;

my %syntax = (
	# General Commands
	JMP		=> [
				qr/($non_negative_dec_or_hex)/, # number Address
				qr/($non_negative_dec_or_hex)$param_separator($non_negative_dec_or_hex)/ # number Address, number MaxAddress
			],
	HDR		=> [ qr/($dec_or_hex)/ ], # number HeaderSize
	...
	# String Commands
	FIXEDLENGTH	=> [ qr/($non_negative_dec_or_hex)$param_separator($non_negative_dec_or_hex_byte)/ ], # number Length, number FillCharacter
	STRINGALIGN	=> [ qr/($positive_dec_or_hex)/ ], # number AlignValue
	...
	CREATEPTR	=> [ qr/($identifier)$param_separator"($addressType)"$param_separator($dec_or_hex)$param_separator($pointerSize)/ ], # custompointer PtrName, string AddressType, number Offsetting, number PtrSize
	WRITE		=> [
				qr/($identifier)$param_separator($non_negative_dec_or_hex)/, # custompointer Ptr, number Address / pointertable PtrTbl, number PtrNum / embpointertable PtrTbl, number PtrNum / counter Ctr, number Address
				qr/($identifier)/ # pointertable PtrTbl / pointerlist List / embpointertable PtrTbl
			],
	PTRTBL		=> [ qr/($identifier)$param_separator($non_negative_dec_or_hex)$param_separator($positive_dec_or_hex)$param_separator($identifier)/ ], # pointertable PtrTbl, number Start, number Offsetting, custompointer Ptr
	...
	W32		=> [
				qr/($identifier)$param_separator($non_negative_dec_or_hex)/, # custompointer Ptr, number Address / counter Ctr, number Address
				qr/($non_negative_dec_or_hex)/ # number Address
			],
	W24		=> [
				qr/($identifier)$param_separator($non_negative_dec_or_hex)/, # custompointer Ptr, number Address / counter Ctr, number Address
	...
	EMBTYPE		=> [ qr/"($addressType)"$param_separator($pointerSize)$param_separator($dec_or_hex)/ ], # string AddressType, number Size, number Offsetting
	EMBSET		=> [ qr/($non_negative_dec_or_hex)/ ], # number PointerNum
	EMBWRITE	=> [ qr/($non_negative_dec_or_hex)/ ], # number PointerNum
	EMBPTRTBL	=> [ qr/($identifier)$param_separator($positive_dec_or_hex)$param_separator($identifier)/ ], # embpointertable PtrTbl, number PtrCount, custompointer Ptr
	# Multifile Commands
	...
	# My Atlas addtions
	AUTOCMD		=> [ qr/($non_negative_dec_or_hex)$param_separator(.*)/ ], # number Address, any other command
	CONTINUOUSFILE	=> [ qr/"($boolean)"/ ], # boolean Continuous
	CREATECTR	=> [ qr/($identifier)$param_separator($positive_dec_or_hex)$param_separator($dec_or_hex)/ ], # counter Ctr, number Size, number Value
	INC		=> [ qr/($identifier)$param_separator($dec_or_hex)/ ], # counter Ctr, number Amount
	PASCALTYPE	=> [ qr/"($pascalType)"/ ], # string PascalType
	SKIP		=> [ qr/($dec_or_hex)/ ], # number Offsetting
...
# $init: whether $command is an initializing command or not
sub check_var {
	...
	if (!$abcde::Atlas::userVars{$varName}) { # no #VAR yet
		...
		die("Attempt to #$command using variable '$varName' of type '$abcde::Atlas::userVars{$varName}->{varType}'!\n");
	} elsif (!$init && !$abcde::Atlas::userVars{$varName}->{value}) { # #VAR done, but trying to use it before initializing
		die("Attempt to #$command using undefined variable '$varName'!\n");
	} elsif ($init && $abcde::Atlas::userVars{$varName}->{value}) { # #VAR done and initialized, but attempting to re-initialize it
...
sub write_low_level_pointer {
	...
	if (scalar(@$args) == 2) {
		$varName = shift(@$args);
		$class->check_var($command, $varName, ['COUNTER', 'CUSTOMPOINTER']);
		$obj = $abcde::Atlas::userVars{$varName};
	}
	my $writeAddress = $class->parse_number(shift(@$args));
...
sub do_AUTOCMD {
	...
	my $address = $class->parse_number(shift(@$args));
	my $line = shift(@$args);
	if ($line =~ /^[ \t]*#($commands)\(/) { # Atlas' parser actually accepts anything in place of (, but then it crashes
		my $command = $1;
		foreach my $signature (@{$syntax{$command}}) {
...
sub do_CREATEPTR {
	...
	$abcde::Atlas::userVars{$varName}->{value} = {
		...
		offset => $class->parse_number($offset),
		size => $size,
	};
}
...
sub do_EMBPTRTBL {
	...
	$abcde::Atlas::userVars{$varName}->{value} = {
		...
		numPointers => $class->parse_number($numPointers),
...
sub do_EMBSET {
	...
	my $index = $class->parse_number(shift(@$args));
	if (!$abcde::Atlas::settings{embeddedPointers}) {
...
sub do_FIXEDLENGTH {
	...
	$abcde::Atlas::settings{fillChar} = chr($class->parse_number($fillChar));
}
...
sub do_HDR {
	...
	my ($headerSize) = @$args;
	$abcde::Atlas::settings{headerSize} = $class->parse_number($headerSize);
}
...
sub do_INC {
	...
	$abcde::Atlas::userVars{$varName}->{value}->{value} = ($abcde::Atlas::userVars{$varName}->{value}->{value} + $class->parse_number($amount)) & $abcde::Atlas::userVars{$varName}->{value}->{mask};
}
...
sub do_JMP {
	...
	$newAddress = $class->parse_number($newAddress);
	...
	if (scalar(@$args)) {
		...
		$abcde::Atlas::settings{maxAddress} = $class->parse_number($maxAddress);
	} else {
...
sub do_PTRLIST {
	...
	while (my $line = <FH>) {
		...
		if ($line =~ /^$non_negative_dec_or_hex$/) {
			push(@lines, $class->parse_number($line));
		} else {
...
sub do_PTRTBL {
	...
	$abcde::Atlas::userVars{$varName}->{value} = {
		startAddress => $class->parse_number($startAddress),
		offsetting => $class->parse_number($offsetting),
		customPointer => $abcde::Atlas::userVars{$customPointerName}->{value},
		pointerIndex => 0,
	};
}
...
sub do_STRINGALIGN {
	...
	my ($stringAlign) = @$args;
	$abcde::Atlas::settings{stringAlign} = $class->parse_number($stringAlign);
}
...
sub do_WRITE {
	...
	if ($abcde::Atlas::userVars{$varName}->{varType} eq 'COUNTER') {
		...
		if ($abcde::Atlas::userVars{$varName}->{varType} eq 'CUSTOMPOINTER') {
			...
			$pointerAddress = $class->parse_number(shift(@$args));
		} else {
			...
			if ($abcde::Atlas::userVars{$varName}->{varType} eq 'POINTERLIST') {
				...
				if (scalar(@$args)) {
					$obj->{pointerIndex} = $class->parse_number(shift(@$args));
				}
				$pointerAddress = $obj->{startAddress} + $obj->{offsetting} * $obj->{pointerIndex};
