	...
	'METHOD' => {
		...
		value_deps => {
			...
			'RAW' => {
				...
				forbidden => ['POINTER TABLE START', 'POINTER TABLE STOP', 'POINTER SIZE', 'POINTER SPACE', 'POINTER ENDIAN', 'ATLAS PTRS', 'BASE POINTER', 'STRINGS PER POINTER'],
			},
		}
	},
...
sub processBlocks {
	...
	foreach my $block_num (0..scalar(@$blocks) - 1) {
		...
		if ($block->{'ATLAS PTRS'} eq 'Yes') {
			...
			my $prev_table = ($block_num == 0 || $output_switch eq '-m' ? undef : defined($blocks->[$block_num - 1]->{'TABLE ID'}) ? $abcde::tablesByID{$blocks->[$block_num - 1]->{'TABLE ID'}} : $abcde::tablesByFileName{$blocks->[$block_num - 1]->{'TABLE'}}->[0]);
			if (!$prev_table || $prev_table->{AtlasID} ne $table->{AtlasID}) {
